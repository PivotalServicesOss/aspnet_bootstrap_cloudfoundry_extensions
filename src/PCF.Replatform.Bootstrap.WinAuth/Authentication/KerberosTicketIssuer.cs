    ...
    public class KerberosTicketIssuer : ITicketIssuer
    {
        ...
        public AuthenticationTicket Authenticate(string base64Token)
        {
            if (string.IsNullOrWhiteSpace(configuration["PRINCIPAL_PASSWORD"])
                    || configuration["PRINCIPAL_PASSWORD"] == AuthConstants.PRINCIPAL_PASSWORD_FROM_CREDHUB)
            {
                throw new ArgumentNullException($"PRINCIPAL_PASSWORD is not set! 1. Set as an environment variable, 2. Set it in credhub with key `principal_password` so that it will be pulled automatically using placeholder resolver `{AuthConstants.PRINCIPAL_PASSWORD_FROM_CREDHUB}`");
            }
